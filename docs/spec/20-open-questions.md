...
When `LOCAL-3f2a` → `WHL-043`, scan all `$TSK_REPO/issues/*.md` for references to `LOCAL-3f2a` in any frontmatter field (`blocks:`, `related:`, `parent:`) and update them atomically before git commit.
...
Pass `host:` from `tsk.yaml` remote entry to `glab` via `GITLAB_HOST` env variable or `--hostname` flag. Document the glab authentication setup for self-hosted instances.
...
~~Symlinks use relative paths (`../../../issues/WHL-042.md`). If the views directory depth changes (new view type added deeper), symlink paths must be recalculated.~~ Views now live in `$XDG_CACHE_HOME/tsk/views/` and use **file copies** from `$TSK_REPO/issues/<ID>.md`. Path depth is irrelevant for copies. If `$TSK_REPO` moves, `tsk view` rebuilds everything.
...
**[RESOLVED] `.REMOTE.md` files in `$TSK_REPO` git.**
`.REMOTE.md` files are committed to `$TSK_REPO` so conflict state survives host switches and `$TSK_REPO` git pull/push. They are excluded from views and `tsk ls` by filtering on the `.REMOTE.md` suffix.

**[RESOLVED] Recurring task `last_run` merge conflict.**
When `tsk.yaml` is merged across hosts, `last_run` fields may conflict if both hosts ran `tsk recur run` independently. Resolution: keep the later date. The dedup check in `tsk recur run` (title pattern match) prevents duplicate issues even if `last_run` is stale.
