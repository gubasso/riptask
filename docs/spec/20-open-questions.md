...
When `LOCAL-3f2a` → `WHL-043`, scan all `$RIPTASK_REPO/issues/*.md` for references to `LOCAL-3f2a` in any frontmatter field (`blocks:`, `related:`, `parent:`) and update them atomically before git commit.
...
~~Symlinks use relative paths (`../../../issues/WHL-042.md`). If the views directory depth changes (new view type added deeper), symlink paths must be recalculated.~~ Views now live in `$XDG_CACHE_HOME/riptask/views/` and use **file copies** from `$RIPTASK_REPO/issues/<ID>.md`. Path depth is irrelevant for copies. If `$RIPTASK_REPO` moves, `tsk view` rebuilds everything.
...
**[RESOLVED] `.REMOTE.md` files in `$RIPTASK_REPO` git.**
