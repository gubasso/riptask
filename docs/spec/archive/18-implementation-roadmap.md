...
- `tsk.yaml` config loading
- bash completions
- `tsk hooks install` / `update` / `status` / `uninstall` + pre-commit validation hook (see [25 — TSK_REPO Data Repository Hooks](25-tsk-repo-hooks.md))
...
- `config set` is a write operation but belongs here because it only modifies `tsk.yaml`, not issue files
