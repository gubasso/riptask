...
- `$TSK_REPO` (`~/.local/share/tsk/`) is a **passive database**. It is never the working directory for `tsk` commands.
...
- All `tsk` state lives in `$TSK_REPO` — nothing is written back to project repos
...
3. infer remote type from host:
   ...
   └── anything else               → type: local  (SoT: $TSK_REPO)
       (codeberg, gitea, gitolite, self-hosted, etc.)

4. look up project in $TSK_REPO/tsk.yaml remotes[]
...
| Other remote (codeberg, gitolite…) | local | `$TSK_REPO` git only |
| No remote | local | `$TSK_REPO` git only |

For `type: local` projects, there is no bidirectional sync against a remote issue tracker. Issues are managed entirely within `$TSK_REPO` and shared across hosts via `$TSK_REPO` git push/pull. IDs are permanent (not provisional) since there is no remote to assign a "real" ID.

### First-run registration

When a project is not yet in `tsk.yaml`, `tsk` prompts for configuration and registers it. The remote type is auto-detected from the host — not prompted:
...
2. **Config load** — on every `tsk` invocation that parses `tsk.yaml`, validate that all `project_prefix` values are unique. If a duplicate is found (e.g. from a manual edit or git merge), abort with a clear error:

```
Error: duplicate project_prefix "WHL" in tsk.yaml (remotes "wormhole-router" and "other-project").
Fix tsk.yaml before continuing.
```

Writes to `$TSK_REPO/tsk.yaml`:
...
Normalization and type inference are both in `lib/detect.sh`. A single string comparison against the stored slug is sufficient — no need to enumerate URL variants in `tsk.yaml`.
...
The inferred type is shown during registration for confirmation but is not prompted. The `type` field is stored in `tsk.yaml` and used for all subsequent operations. If the heuristic is wrong (e.g. a self-hosted GitHub Enterprise on a non-`github.com` domain), the user can override `type` manually in `tsk.yaml`.
