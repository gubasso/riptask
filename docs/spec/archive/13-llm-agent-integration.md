...
ai_call() {
    ...
    local user_prompt="$2"
    local config_model
    config_model="$(yq '.ai.model // ""' "$RIPTASK_REPO/riptsk.yaml" 2>/dev/null || true)"
...
Model precedence is: `$RIPTASK_AI_MODEL` environment variable > `ai.model` in `riptsk.yaml` > hardcoded default.
