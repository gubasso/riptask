...
- **Local projects** (`type: local` — non-GitHub/GitLab remotes or no remote) are **skipped by `tsk sync`**. They have no remote issue tracker to sync against. Cross-host sharing for local projects is handled entirely via `$RIPTASK_REPO` git push/pull
...
update $XDG_CACHE_HOME/riptask/remote_state.json
regenerate views/
```
...
After writing `.REMOTE.md` and adding `conflict:` metadata, `tsk sync pull` commits these files to `$RIPTASK_REPO` with `RIPTASK_HOOK_ALLOW_REMOTE=1` (see [25 — Hooks](25-tsk-repo-hooks.md)).
...
for each issue in dirty_issues:
    if local_id is set (never pushed):
        ...
        update $XDG_CACHE_HOME/riptask/id_map.json: LOCAL-xxxx → <PREFIX>-<remote_id>
...
`~/.cache/riptask/remote_state.json` — the sync baseline for conflict detection and the diff baseline for `tsk sync status`.
...
`~/.cache/riptask/id_map.json` — tracks local-to-remote ID assignments for the file rename flow.
