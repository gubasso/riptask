...
Views live in `$XDG_CACHE_HOME/riptask/views/` (default `~/.cache/riptask/views/`), outside `$RIPTASK_REPO`. View files are **copies** of `$RIPTASK_REPO/issues/<ID>.md`. If `$RIPTASK_REPO` moves, `tsk view` rebuilds everything.
...
tree ~/.cache/riptask/views/kanban/penguin-chrono-labs/

# What's in progress?
ls ~/.cache/riptask/views/kanban/penguin-chrono-labs/in-progress/

# Read an issue directly (view copy has the same content)
cat ~/.cache/riptask/views/kanban/penguin-chrono-labs/in-progress/01-WHL-042.md

# Edit the canonical issue (not the view copy)
tsk edit WHL-042

# All issues for a project
ls ~/.cache/riptask/views/projects/wormhole-router/

# Cross-project org view
tree ~/.cache/riptask/views/orgs/penguin-chrono-labs/
```
