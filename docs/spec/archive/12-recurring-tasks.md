...
Recurring tasks are definitions in `riptask.yaml` that auto-generate new issues on a schedule. They are not issues themselves — they are templates + scheduling metadata.

### `riptask.yaml` recurring definition
...
5. Update `last_run: <today>` in `riptask.yaml`
6. Regenerate views
...
**Race condition**: If both hosts run `tsk recur run` before syncing `riptask.yaml` via `$RIPTASK_REPO` git, both see `last_run: null` (or same old date) and instantiate the same recurring task — creating duplicates.

**Dedup strategy**: Before creating an instance, check if `issues/` already contains a file with `recurring: <recur-id>` and a title matching the current period's `title_pattern` expansion. If a match exists, update `last_run` in `riptask.yaml` without creating a duplicate. This makes `tsk recur run` idempotent across hosts, not just within a single host.

**Git merge of `last_run`**: When `riptask.yaml` is merged across hosts, conflicting `last_run` values should be resolved by keeping the later date (both hosts created the same instance — the later `last_run` is correct).
