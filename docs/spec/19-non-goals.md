...
- **Team collaboration tooling** — designed for a single engineer across multiple machines, but usable by a small team with a simple convention: each developer works on one task at a time (task-per-dev partitioning). Conflicts are detected and preserved for manual resolution (see [03 — Design Decisions §3.5](03-design-decisions.md)). What `tsk` will *not* build: shared dashboards, team notifications, access control, or real-time co-editing. Multi-host sync is supported via `tsk session` and `$TSK_REPO` git (see [11 — Multi-Host Workflow](archive/11-multi-host-workflow.md))
- **Three-way merge for conflicts** — conflicts are detected and preserved for manual resolution (see [03 — Design Decisions §3.5](03-design-decisions.md)), but `tsk` does not attempt automatic three-way merging of frontmatter or body content
