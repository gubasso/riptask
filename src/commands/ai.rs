...
pub fn summarize(paths: &AppPaths, args: SummarizeArgs) -> Result<(), TskError> {
...
pub fn ask(paths: &AppPaths, args: AskArgs) -> Result<(), TskError> {
    paths.require_initialized()?;
    let config = load_config(paths.config_path().as_std_path()).map_err(TskError::Other)?;
...
pub fn generate_body(paths: &AppPaths, title: &str, project: &str) -> Result<String, TskError> {
    paths.require_initialized()?;
    let config = load_config(paths.config_path().as_std_path()).map_err(TskError::Other)?;
    if !config.ai.enabled || !config.ai.features.new_body_gen {
        return Err(TskError::General("AI features disabled".into()));
    }
    let backend = backend(&config)?;
    backend.generate_body(&format!("Title: {title}\n\nContext:\n{project}"))
}

fn backend(config: &crate::config::Config) -> Result<CommandAiBackend, TskError> {
    optional_backend(config)?.ok_or_else(|| TskError::General(AI_BACKEND_MISSING.into()))
}

fn optional_backend(config: &crate::config::Config) -> Result<Option<CommandAiBackend>, TskError> {
    let model = config
        .ai
        .model
        .clone()
        .or_else(|| std::env::var("TSK_AI_MODEL").ok())
