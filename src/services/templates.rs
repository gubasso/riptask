...
impl<'a> TemplateService<'a> {
    ...
    pub fn list_names(&self) -> Result<Vec<String>, RiptaskError> {
    ...
    pub fn validate_file(&self, path: &Path) -> Result<(), RiptaskError> {
        let content = fs::read_to_string(path)
        ...
        let Some(template_name) = value.get("template_name").and_then(|value| value.as_str())
            ...
            return Err(RiptaskError::Config("template_name is required".into()));
        };
        if template_name.is_empty() {
            return Err(RiptaskError::Config("template_name is required".into()));
        }
        if let Some(state) = value.get("default_status").and_then(|value| value.as_str()) {
            parse_state(state).map_err(|error| RiptaskError::Config(error.to_string()))?;
        }
        if let Some(priority) = value
            .get("default_priority")
            .and_then(|value| value.as_str())
        {
            map_priority(priority).map_err(|error| RiptaskError::Config(error.to_string()))?;
        }
        Ok(())
    }
}
